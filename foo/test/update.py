...
def updatexd():
    try:
        # 定义压缩包的URL和保存路径
        zip_file_url = "https://github.com/MetaCubeX/metacubexd/archive/refs/heads/gh-pages.zip"
        save_path = "xd.zip"
        # 使用requests模块下载压缩包
        ...
        os.remove(save_path)
        # 定义源文件夹和目标文件夹的路径
        src_dir = "tmp/metacubexd-gh-pages/"
...
def updatecore():
    try:
        ...
        download_version = response.json()["assets"][0]["name"].split(
            "arm64-")[1].split(".gz")[0]
        os.makedirs("./tmp", exist_ok=True)
        file_name = f"clash.meta-windows-amd64-{download_version}.zip"
        file_path = os.path.join("./tmp", file_name)
        r = requests.get(
            f"{download_url}/Prerelease-Alpha/{file_name}", stream=True)
